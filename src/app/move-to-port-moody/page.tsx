...
export default function MovePage() {
  return (
    <>
      <ImageHero imageSrc="/images/hero/port-moody-hero-summer.jpg" imageAlt="Summer view across Burrard Inlet in Port Moody with full green leaves, calm water, and forested mountains." title="Thinking about moving to Port Moody? Start with the lifestyle, then the real estate." subtitle="Trails, waterfront, breweries, SkyTrain, schools, hillsides, density, and neighbourhood feel all matter when deciding where to live." primaryCta={{ label: "Get the relocation starter guide", href: "#relocation-form" }} secondaryCta={{ label: "Ask about living here", href: "#relocation-form" }} />
      <Section title="Why people move here" tone="white"><div className="grid gap-6 lg:grid-cols-[1fr_1.15fr]"><ContextImage src="/images/phase1/port-moody-forest-path.jpg" alt="Forest trail in Port Moody with sunlight filtering through trees." /><div className="grid gap-3 sm:grid-cols-2">{reasons.map((item) => <div key={item} className="rounded-md border border-softBorder bg-mist p-4 text-sm font-medium text-charcoal">{item}</div>)}</div></div></Section>
      ...
      <Section title="Moving from Vancouver to Port Moody" intro="Expect a different mix of space, price, commute, transit, schools, and community feel. The right answer depends on your daily pattern, not just the map." tone="white"><ContextImage src="/images/phase1/port-moody-regional-view.jpg" alt="View from Port Moody toward the Metro Vancouver skyline with trees and hillside homes in the foreground." /></Section>
